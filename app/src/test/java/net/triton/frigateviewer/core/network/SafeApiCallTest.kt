...
class SafeApiCallTest {
    ...
    private interface TestApi {
        @GET("/thing") suspend fun thing(): retrofit2.Response<String>
    }
    ...
    @Test fun `200 with body returns Success`() = runBlocking {
    ...
    @Test fun `401 with HTML body returns HttpError without crashing`() = runBlocking {
        server.enqueue(
    ...
    @Test fun `500 with HTML body returns HttpError`() = runBlocking {
        server.enqueue(
    ...
    @Test fun `socket close returns NetworkError`() = runBlocking {
    ...
    @Test fun `204 empty body returns ParseError`() = runBlocking {
